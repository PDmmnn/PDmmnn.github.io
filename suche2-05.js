...
function processResults(data, query) {
    ...
    if (data.items) {
        data.items.forEach(item => {
            ...
            // Extracting search terms related to 'foerderhöhebar'
            const searchTermsFoerderhoehebar = extractSearchTerms('foerderhöhebar');
            ...
            resultItem.innerHTML = `
                ...
                <div class="result-details">
                    ...
                    <span>Found Terms: ${searchTermsFoerderhoehebar.join(', ')}</span>
                </div>
            `;
...
// Function to extract search terms for a specific input field
function extractSearchTerms(inputId) {
    const inputField = document.getElementById(inputId);
    const inputText = inputField.value.trim().toLowerCase();
    return inputText.split(',').map(term => term.trim());
}
